...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SPLA);
    ...
    if (!settings.contains("nAnonymizeSplashFarmsAmount"))
        settings.setValue("nAnonymizeSplashFarmsAmount", 1000);

    nAnonymizeSplashFarmsAmount = settings.value("nAnonymizeSplashFarmsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSplashFarmsAmount"))
        SoftSetArg("-anonymizesplashfarmsamount", settings.value("nAnonymizeSplashFarmsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in splashfarms.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSplashFarmsAmount:
            return QVariant(nAnonymizeSplashFarmsAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSplashFarmsAmount:
            nAnonymizeSplashFarmsAmount = value.toInt();
            settings.setValue("nAnonymizeSplashFarmsAmount", nAnonymizeSplashFarmsAmount);
            emit anonymizeSplashFarmsAmountChanged(nAnonymizeSplashFarmsAmount);
            break;
        case CoinControlFeatures:
