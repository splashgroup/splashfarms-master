...
#include "ui_zsplacontroldialog.h"
...
std::set<std::string> ZSplaControlDialog::setSelectedMints;
std::set<CMintMeta> ZSplaControlDialog::setMints;

ZSplaControlDialog::ZSplaControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSplaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSplaControlDialog::~ZSplaControlDialog()
{
    delete ui;
}

void ZSplaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSplaControlDialog::updateList()
{
...
void ZSplaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSplaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSpla_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSplaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSplaControlDialog::GetSelectedMints()
{
...
void ZSplaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
