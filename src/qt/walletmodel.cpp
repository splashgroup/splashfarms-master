...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered splashfarms address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SPLA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal splashfarms:URI (splashfarms:XyZ...?message=example)
            {
...
static void NotifyzSPLAReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSPLAReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSPLAReset.connect(boost::bind(NotifyzSPLAReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSPLAReset.disconnect(boost::bind(NotifyzSPLAReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsplaTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
