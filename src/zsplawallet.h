...
#ifndef SPLASHFARMS_ZSPLAWALLET_H
#define SPLASHFARMS_ZSPLAWALLET_H
...
public:
    CzSPLAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSPLA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSPLA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SPLASHFARMS_ZSPLAWALLET_H
