...
public:
    ...
    bool WriteZSPLASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSPLASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSPLASeed_deprecated(uint256& seed);
    bool EraseZSPLASeed();
    bool EraseZSPLASeed_deprecated();

    bool WriteZSPLACount(const uint32_t& nCount);
    bool ReadZSPLACount(uint32_t& nCount);
