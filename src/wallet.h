...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 SPLA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSPLA send
enum ZerocoinSpendStatus {
    ZSPLA_SPEND_OKAY = 0,                            // No error
    ZSPLA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSPLA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSPLA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSPLA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSPLA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSPLA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSPLA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSPLA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSPLA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSPLA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSPLA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSPLA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSPLA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSPLA_SPENT_USED_ZSPLA = 14,                      // Coin has already been spend
    ZSPLA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSPLA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSplaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSPLAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsplaAuto) const;
    ...
    */
    ...
    CzSPLAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSPLATracker> zsplaTracker;
    ...
    void setZWallet(CzSPLAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsplaTracker = std::unique_ptr<CzSPLATracker>(new CzSPLATracker(strWalletFile));
    }

    CzSPLAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZSplaAutoBackups(bool fEnabled)
    {
    ...
    /** zSPLA reset */
    boost::signals2::signal<void()> NotifyzSPLAReset;
