...
CzSPLAWallet::CzSPLAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zspla seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSPLASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSPLASeed_deprecated()) {
                    LogPrintf("%s: Updated zSPLA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zspla seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zspla wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSPLAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSPLACount(nCountLastUsed);
    else if (!walletdb.ReadZSPLACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSPLAWallet::Lock()
{
    seedMaster = 0;
}

void CzSPLAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSPLAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSPLA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSPLAWallet::LoadMintPoolFromDB()
{
...
void CzSPLAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSPLAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSPLAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsplaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSPLAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSPLA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsplaTracker which also adds to database
    pwalletMain->zsplaTracker->Add(dMint, true);
...
void CzSPLAWallet::SeedToZSPLA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSPLAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSPLAWallet::GenerateDeterministicZSPLA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSPLAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSPLAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
