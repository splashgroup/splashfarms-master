...
CzSPLATracker::CzSPLATracker(std::string strWalletFile)
{
...
CzSPLATracker::~CzSPLATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSPLATracker::Init()
{
...
bool CzSPLATracker::Archive(CMintMeta& meta)
{
...
bool CzSPLATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSPLATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSPLATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSPLATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSPLATracker::GetSerialHashes()
{
...
CAmount CzSPLATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSPLATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSPLATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSPLATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSPLATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSPLATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSPLATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSPLATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSPLATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSPLATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSPLATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPLATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPLATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSPLATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSPLATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSPLATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSPLATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzspla from DB\n", __func__, listDeterministicDB.size());
    }
