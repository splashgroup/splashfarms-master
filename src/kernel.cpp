...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSplaStake(spend));
    } else {
        ...
        CSplaStake* splaInput = new CSplaStake();
        splaInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(splaInput);
    }
